  ...
  return (
    <div className="relative min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 overflow-hidden font-sans">
      ...
      <div className="relative z-10 sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-4 text-center text-3xl font-extrabold tracking-tight text-white drop-shadow-md">Create an Account</h2>
        <p className="mt-1 text-center text-sm text-slate-300">Join EventFlow for instant event management</p>
      </div>

      <div className="relative z-10 mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white/70 backdrop-blur-2xl py-8 px-6 shadow-2xl rounded-3xl border border-white/40 sm:px-10 space-y-5">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-700">Account Type / Role</label>
            <div className="mt-1">
              <select {...register('role')} className="block w-full rounded-xl border border-white/60 bg-white/80 backdrop-blur-md px-4 py-2.5 shadow-sm focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm font-semibold text-slate-900 transition-all">
          ...
          <div>
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="bg-white/80 backdrop-blur-md px-3 py-0.5 rounded-full text-slate-600 font-medium">Or register with email</span>
              </div>
            </div>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            <div>
              ...
              <div className="mt-1">
                <input {...register('fullName')} type="text" className="block w-full rounded-xl border border-white/60 bg-white/70 backdrop-blur-md px-4 py-2 text-slate-900 placeholder-slate-400 shadow-sm focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm font-medium transition-all" />
                {errors.fullName && <p className="mt-1 text-xs font-semibold text-rose-600">{errors.fullName.message}</p>}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1">
                <input {...register('email')} type="email" className="block w-full rounded-xl border border-white/60 bg-white/70 backdrop-blur-md px-4 py-2 text-slate-900 placeholder-slate-400 shadow-sm focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm font-medium transition-all" />
            ...
            <div>
              ...
              <div className="mt-1 relative rounded-xl shadow-sm">
                <input {...register('password')} type={showPassword ? 'text' : 'password'} className="block w-full rounded-xl border border-white/60 bg-white/70 backdrop-blur-md px-4 py-2 pr-10 text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm font-medium transition-all" />
            ...
            <div>
              ...
              <div className="mt-1 relative rounded-xl shadow-sm">
                <input {...register('confirmPassword')} type={showConfirmPassword ? 'text' : 'password'} className="block w-full rounded-xl border border-white/60 bg-white/70 backdrop-blur-md px-4 py-2 pr-10 text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm font-medium transition-all" />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute inset-y-0 right-0 flex items-center pr-3 focus:outline-none text-slate-500 hover:text-slate-700">
                  {showConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {errors.confirmPassword && <p className="mt-1 text-xs font-semibold text-rose-600">{errors.confirmPassword.message}</p>}
            </div>
